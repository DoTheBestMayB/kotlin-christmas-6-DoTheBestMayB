...
class InputView(
    ...
    fun readMenu(chef: Chef): List<Menu> {
        outputView.show(REQUEST_INPUT_MENU, true)
        val input = read()
        return changeInputToMenu(input, chef)
    }

    private fun changeInputToMenu(input: String, chef: Chef): List<Menu> {
        val orderedMenu = mutableSetOf<Menu>()
        ...
        for (phrase in input.split(ORDER_INPUT_SPLITTER)) {
            ...
            val (name, size) = phrase.split(ORDER_MENU_FORMAT_SPLITTER)
            // 메뉴의 개수가 1 이상의 숫자가 아닐 때
            checkValidity(size, ORDER_IS_NOT_VALID) { validator.checkSingleMenuSize(it) }

            // 고객이 메뉴를 20개를 초과해서 주문한 경우
            checkValidity(size.toInt() + orderedMenu.size, ORDER_SIZE_IS_OVER) { validator.checkTotalOrderSize(it) }
    ...
    companion object {
        ...
        private const val ORDER_SIZE_IS_OVER = "[ERROR] 메뉴는 한 번에 최대 20개까지만 주문할 수 있습니다."
        private const val DRINK_ONLY_ORDER_IS_NOT_ALLOWED = "[ERROR] 음료만 주문 시, 주문할 수 없습니다."
