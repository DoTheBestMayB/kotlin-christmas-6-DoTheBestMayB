...
- [x] 할인 정보를 나타내는 Benefit 데이터 클래스를 만든다.
  - 이벤트 이름, 할인 받은 가격 2개의 파라미터로 구성되어 있다.
- [x] 증정품 데이터를 저장하는 Gift 데이터 클래스를 만든다.
- [x] 메뉴를 나타내는 Menu Enum 클래스를 만든다.
- [x] 메뉴의 종류를 나타내는 MenuType Enum 클래스를 만든다.
- [x] 주문한 메뉴와 수량을 저장하는 OrderTicket 데이터 클래스를 만든다.
- [x] 주문한 내용과 이벤트 혜택 정보를 담고 있는 Receipt 데이터 클래스를 만든다.
...
- [x] 예약을 받고 관리하는 Planner 클래스를 만든다.
  ...
  - [x] 손님에게 예약 내역을 보여준다.
- [x] Pos(포스기) 클래스를 만든다.
  - [x] 주문 정보를 입력하면 주문한 내용과 이벤트 혜택 정보를 담고 있는 Receipt을 반환한다.
- [x] 데이터 검증을 담당하는 Validator 클래스를 만든다.
  - [x] 입력한 문자열을 방문 날짜로 변환하는 함수를 만든다.
  - [x] 입력한 문자열을 주문 정보로 변환하는 함수를 만든다.
...
- [x] 입출력 값이 동일한지 테스트코드를 작성해서 확인한다.


### 3주차 피드백
- [x] 현재 객체의 상태를 보기 위한 로그 메시지 성격이 강하다면 toString()을 통해 구현한다.
  - View에서 사용할 데이터라면 getter 메서드를 통해 데이터를 전달한다.
- [x] 연관성이 있는 상수는 전역으로 선언하는 대신 enum을 활용한다
- [x] val 키워드를 사용해 값의 변경을 막는다
- [x] 객체의 상태 접근을 private 접근자를 사용해서 제한한다
- [x] getter를 사용하는 대신 객체에 메세지를 보내자
...
- [x] 테스트 코드도 코드이므로 리팩터링을 통해 개선해 나가야 한다
